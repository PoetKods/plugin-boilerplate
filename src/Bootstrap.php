...
use Pk\Common\Traits\Requester;
use Pk\Common\Utils\Errors;
use Pk\Config\Classes;
use Pk\Config\I18n;
use Pk\Config\Requirements;
...
final class Bootstrap extends Base {
	...
	public function initClasses() {
		$this->class_list = \apply_filters( 'pk_initialized_classes', $this->class_list );
		foreach ( $this->class_list as $class ) {
			try {
				...
				\do_action( 'pk_class_initialize_failed', $err, $class );
				Errors::wpDie(
					...
					__( 'Plugin initialize failed', 'pk' ),
					__FILE__, $err
				);
			}
		}
	...
	public function getClasses( string $namespace ): array {
		...
		// Errors::writeLog(
		//    [
		//        'title'   => __( 'PK classes are not being loaded by Composer\'s Autoloader' ),
